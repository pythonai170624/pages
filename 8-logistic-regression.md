...
#### ✅ משמעות:
- \( \hat{p}_i \) היא ההסתברות של המודל לכך שהתצפית ה-\( i \) תהיה 1.
- \( y_i \) הוא הערך בפועל — או 0 או 1.
- כאשר \( y_i = 1 \), מקבלים: \( \hat{p}_i \)
- כאשר \( y_i = 0 \), מקבלים: \( 1 - \hat{p}_i \)

הנוסחה חכמה בכך שהיא משתמשת ב-\( y_i \) ו-\( 1 - y_i \) כדי לכסות את שני המקרים באותה נוסחה — בלי תנאים מיוחדים
