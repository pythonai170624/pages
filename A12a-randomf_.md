...
- `bootstrap` → האם לבצע דגימה עם החזרה (Bootstrapping). ברירת מחדל: True.
- `oob_score` → אם True, המודל ישתמש בדוגמאות שלא נבחרו במדגם כדי להעריך את הדיוק (Out-Of-Bag Error) בלי צורך בסט בדיקה נפרד.
