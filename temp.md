...
פונקצית הקרנל, המסומנת לרוב כ-$K(x, y)$, מחשבת את מכפלת הנקודות (dot product) של שני וקטורים $x$ ו-$y$ לאחר שעברו טרנספורמציה למרחב תכונות גבוה יותר, מבלי לחשב במפורש את הטרנספורמציה עצמה:
