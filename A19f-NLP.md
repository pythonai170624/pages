...
3. `X_train_tfidf = vectorizer.fit_transform(X_train)`  
   - `.fit()` → בונה את אוצר המילים מתוך `X_train`  
   - `.transform()` → ממיר את כל הטקסטים למטריצת TF-IDF  
   - התוצאה: מטריצה בגודל (3900, 7245) של ערכים מספריים

4. `X_train_tfidf.shape`  
   מציג את גודל המטריצה שהתקבלה – כלומר כמה טקסטים (שורות) וכמה מילים ייחודיות (עמודות)
