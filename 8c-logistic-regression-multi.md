...
כאשר יש לנו **K קטגוריות** שונות, אנחנו משתמשים בפונקציית Softmax כדי לחשב את ההסתברות שהדוגמה שלנו שייכת למחלקה \( k \).

- \( x = [x_1, x_2, ..., x_n] \): וקטור התכונות של הדוגמה שלנו
- \( \beta_{k0} \): bias (היסט) למחלקה \( k \)
- \( \beta_{k1}, \dots, \beta_{kn} \): משקלים של כל תכונה עבור מחלקה \( k \)
- \( e \): בסיס הלוגריתם הטבעי (≈ 2.718)
- \( K \): מספר הקטגוריות
- המכנה כולל את כל המחלקות האפשריות \( j = 1 \dots K \)
